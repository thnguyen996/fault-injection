...
def remainder2bit_int(remainder, num_bits=127):
  """Turn a tensor with remainders (floats < 1) to mantissa bits.
...
def float2bit_int(f, num_e_bits=8, num_m_bits=23, bias=127., dtype=torch.float32):
  """Turn input tensor into binary.
  ...
  # turn sign into sign-bit
  s = (s * (-1) + 1.) * 0.5
  s = s.unsqueeze(-1).type(torch.int8)

  ## EXPONENT BIT
  e_scientific = torch.floor(torch.log2(f))
  e_decimal = e_scientific + bias
  e = integer2bit_int(e_decimal, num_bits=num_e_bits)

  # pdb.set_trace()
  ## MANTISSA
  m1 = integer2bit_int(f - f % 1, num_bits=num_e_bits)
...
def integer2bit_int(integer, num_bits=8):
  """Turn integer tensor to binary representation.
  ...
  exponent_bits = -torch.arange(-(num_bits - 1), 1).type(dtype)
  exponent_bits = exponent_bits.repeat(integer.shape + (1,))
  out = integer.unsqueeze(-1).type(torch.int8) / 2 ** exponent_bits
  out = (out - (out % 1)) % 2
  return out
